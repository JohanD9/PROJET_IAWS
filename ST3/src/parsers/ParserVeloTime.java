...
public class ParserVeloTime
{
	...
	public void parse(String lieuOrigine, String villeOrigine, String lieuArrivee, String villeArrivee) throws IOException, JSONException
	{
		String http = new String("http://maps.googleapis.com/maps/api/directions/json?origin="
				...
				+ lieuArrivee + "%20" + villeArrivee + "&sensor=false&avoid=highways|ferries&mode=bicycling&alternatives=false");
		
		LaunchRequest requestVeloTime = new LaunchRequest(http);
		
		String jsonVeloTime = requestVeloTime.get();
		
		JSONObject jsonObject = new JSONObject(jsonVeloTime);
